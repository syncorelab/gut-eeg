...
"""
Run random hyperparameter search for the CNN-LSTM classifier on the
preprocessed tensor data, then evaluate the best trial on a separate
holdout subject set.
...
def create_experiment_dir(root: str = "results") -> Path:
    """
    Create a folder for one full random-search run.
...
def create_trial_dir(exp_dir: Path, trial_idx: int) -> Path:
    """Create a folder for one hyperparameter trial inside the experiment."""
...
def main() -> None:
    ...
    # Save the search space used for this experiment.
    save_json(HPARAM_DISTS, exp_dir / "hparam_distributions.json")

    holdout_subject = None
    best_trial_info = None

    # Sample hyperparameters, train a model, and save outputs for each
    # valid trial.
    for trial in range(N_TRIALS):
        ...
        # Initialize the model using the sampled architecture settings.
        model = GUTNet(**MODEL_HPARAMS)
        model.to(device)

        # Train the model and collect outputs needed for evaluation plots.
        ...
        # Save a compact summary for this trial.
        save_json(
            {
                "trial": trial,
    ...
    # Rebuild the best model and load its saved weights.
    ...
    # Save summary metrics and raw predictions for the holdout evaluation.
    save_json(
        {
